...
class PickupStationControlScreen extends StatefulWidget {
  const PickupStationControlScreen({super.key});

  @override
  _PickupStationControlScreenState createState() =>
      _PickupStationControlScreenState();
}

class _PickupStationControlScreenState
    extends State<PickupStationControlScreen> {
...
/*
  void _updatePickupStation(String key, String newValue) {
    databaseReference
        .child("Pickup Stations")
        .update({key: newValue}).then((_) {
      setState(() {
        pickupStations[key] = newValue;
      });
      //print("$key pickup station updated successfully.");
    }).catchError((error) {
      //print("Failed to update $key pickup station: $error");
    });
  }
*/
